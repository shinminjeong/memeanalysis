...
from mmkg.model.download import maybe_download_and_extract
from mmkg.model.cache import cache
import os
import sys
...
data_dir = "/Users/minjeongshin/Work/mmkg/data/imagenet"
...
class Inception:
    ...
    def _create_feed_dict(self, image_path=None, image=None):
        ...
        if image is not None:
            ...
            image_data = tf.compat.v2.io.gfile.GFile(image_path, 'rb').read()
    ...
    def get_labels(self, pred, k=10, only_first_name=True):
        idx = pred.argsort()
        top_k = idx[-k:]
    ...
    def transfer_softmax(self, image_path=None, image=None):

        # Create a feed-dict for the TensorFlow graph with the input image.
        feed_dict = self._create_feed_dict(image_path=image_path, image=image)
        pool3, softmax0 = self.session.run([self.transfer_layer, self.y_pred], feed_dict=feed_dict)

        # Reduce to a 1-dim array.
        pool3 = np.squeeze(pool3)
        softmax0 = np.squeeze(softmax0)

        return pool3, softmax0
